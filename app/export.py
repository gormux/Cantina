...
def create_xls(booking_data):
    ...
    for user in all_users:
        ...
        worksheet.write_formula(row, all_users[user], f'=SUM({col_name}2:{col_name}{row})', value=value)

    # Next per day
    for row in range(2, row + 1):
        worksheet.write_formula(row - 1, i, f'=SUM(B{row}:{col_name_end}{row})', value=date_values[row - 1])
