...
def admin():
    ...
    if category == 'users_add':
        ...
        if request.method == 'GET':
            return render_template('admin.html', category=category, periods=calendar.periods)
        else:
        ...
        if request.method == 'GET':
            ...
            for item in request.form:
                c = db.session.query(Configuration).filter(Configuration.config_key == item).first()
                ...
                if item in ['mail_cantine_time', 'mail_garderie_time']:
                    ...
                    m = db.session.query(Configuration).filter(Configuration.config_key == conf_name).first()
                    args = {}
...
def savebooking():
    ...
    q = db.session.execute(f'SELECT * FROM booking_{category} WHERE username = "{current_user.name}";')
